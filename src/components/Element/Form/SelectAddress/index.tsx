...
const SelectAddress: FunctionComponent<Props> = ({ item }) => {
  ...
  return (
    <Event.Drag el={item} isFormItem disable={selectedElement?.id === item?.formId || selectedId !== item?.id}>
      ...
      >
        ...
        >
          <option key="none" value="none">Tỉnh / thành</option>
        </select>
        <select
          style={{
            opacity: `${(item?.transform?.opacity ? item?.transform?.opacity / 100 : 100)}`,
          ...
          className={styleBorder}
        >
          <option key="none" value="none">Quận/huyện</option>
        </select>
        <select
          style={{
            opacity: `${(item?.transform?.opacity ? item?.transform?.opacity / 100 : 100)}`,
          ...
          className={styleBorder}
        >
          <option key="none" value="none">Xã/phường</option>
        </select>
