...
const HeightSection: FunctionComponent<Props> = ({ el, type }) => {
    ...
    if (type === 'elements') {
      const elements = html?.elements?.map((item: any) => {
        if (item?.id === el?.id) {
          return {
            ...item,
            widthElement: parseInt(height, 10),
          }
        }
        return item
      })
  ...
  return (
    <div className="d-flex align-items-center justify-content-between my-2">
      <label>Chiều rộng item</label>
      <div className="__collapse__content">
        <input type="number" value={widthValue} onChange={e => changeHeightSection(e.target.value)} />
