...
const Perspective: FunctionComponent<Props> = ({ el, type }) => {
  const dispatch = useDispatch()

  const html = useSelector((state: ApplicationState) => state.builder.html)
  const history = useSelector((state: ApplicationState) => state.builder.history)
  const [perspective, setPerspective] = useState(el?.transform?.perspective)
  ...
  return (
    <div className="d-flex align-items-center justify-content-between my-2">
      <label>Chiều sâu 3D</label>
      <div className="__collapse__content">
        <input type="number" value={perspective} onChange={e => onChangeValue(e.target.value)} />
