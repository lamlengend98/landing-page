...
const SkewX: FunctionComponent<Props> = ({ el, type }) => {
  const dispatch = useDispatch()

  const html = useSelector((state: ApplicationState) => state.builder.html)
  const history = useSelector((state: ApplicationState) => state.builder.history)

  const [skewX, setSkewX] = useState(el?.transform?.skewX)
  ...
  return (
    <div className="d-flex align-items-center justify-content-between my-2">
      <label>Nghiêng ngang</label>
      <div className="__collapse__content">
        <input type="number" value={skewX} onChange={e => onChangeValue(e.target.value)} />
