...
const Grayscale: FunctionComponent<Props> = ({ el, type }) => {
  const dispatch = useDispatch()

  const html = useSelector((state: ApplicationState) => state.builder.html)
  const history = useSelector((state: ApplicationState) => state.builder.history)

  const [grayscale, setGrayscale] = useState(el?.filter?.grayscale)
  ...
  return (
    <div className="d-flex align-items-center justify-content-between my-2">
      <label>Trắng đen</label>
      <div className="__collapse__content">
        <input min={0} max={100} type="number" value={grayscale} onChange={e => onChangeValue(e.target.value)} />
